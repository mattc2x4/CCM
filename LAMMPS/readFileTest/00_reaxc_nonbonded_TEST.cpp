                         ...
                         reax_list **lists, output_controls * /*out_control*/ )
{
  ...
  for (int restk = 0; restk < nRowsrest; restk++) {
	...
	testFile<<"id1 compared to: "<< system->my_atoms[i].orig_id<<"   id2 compared to "<<system->my_atoms[i].orig_id;
    for ( i = 0; i < (natoms-1); i++ ) {
		if ( (id1rest == system->my_atoms[i].orig_id) or (id2rest == system->my_atoms[i].orig_id) ) {				//this line is puzzling, have to figure out what happens here.
			for ( j = (i+1); j < natoms; j++) {
				if ( (id1rest == system->my_atoms[j].orig_id) or (id2rest == system->my_atoms[j].orig_id) ) {
                    ...
                    testFile<< "dxrest: "<<system->my_atoms[i].x[0]<<" - "<<system->my_atoms[j].x[0]<<"\n";
                    testFile<< "dyrest: "<<system->my_atoms[i].x[1]<<" - "<<system->my_atoms[j].x[1]<<"\n";
                    testFile<< "dzrest: "<<system->my_atoms[i].x[2]<<" - "<<system->my_atoms[j].x[2]<<"\n";
