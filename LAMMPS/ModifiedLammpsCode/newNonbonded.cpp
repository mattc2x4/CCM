                         ...
                         reax_list **lists, output_controls * /*out_control*/ )
{
  ...
  for (int restk = 0; restk < nRowsrest; restk++) {
		...
		while (id1Found == false || id2Found == false) {
			if (id1rest == system->my_atoms[i].orig_id && !id1Found) {		//found id1, and haven't found it yet.
				ind_id1 = i;
				id1Found = true;
			}
			if (id2rest == system->my_atoms[i].orig_id && !id2Found) {		//found id2, and haven't found it yet.
				ind_id2 = i;
				id2Found = true;
			}
