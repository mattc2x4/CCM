...
#        the job:
#
#SBATCH --job-name=pyLammpsExtended
#
...
#
# PATH=$PATH:/work/ccm_gillespi/sw/lammps/13Mar2018-ccm_gillespi-python-reaxc-reaxff/bin
#
# vpkg_require python/2.7.15 openmpi/3.1.1:intel # fftw/3.3.8:intel ffmpeg/4.0:intel
# export PYTHONPATH=${PYTHONPATH}:/work/ccm_gillespi/sw/lammps-restraint/lammps-12Dec18/python
# export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:/work/ccm_gillespi/sw/lammps-restraint20Jun19/lammps-12Dec18/src
# export PYTHONPATH=${PYTHONPATH}:/work/ccm_gillespi/sw/lammps/13Mar2018-ccm_gillespi-python-reaxc-reaxff/lib/python2.7/site-packages
vpkg_require lammps/20Jun2019:ccm_gillespi,restraint
#
...
PYTHON_INPUT="MASTER-restraint-source-test.py"
PYTHON_OUTPUT="python.out"
...
if [ -z "$PYTHON_INPUT" ]; then
  echo "ERROR:  PYTHON_INPUT was not set"
  exit 1
fi
if [ ! -f "$PYTHON_INPUT" ]; then
  echo "ERROR:  PYTHON_INPUT does not exist: $PYTHON_INPUT"
  exit 1
fi
echo
...
if [ -z "$PYTHON_OUTPUT" ]; then
  ${UD_MPIRUN} python "$PYTHON_INPUT"
  mpi_rc=$?
else
  ${UD_MPIRUN} python "$PYTHON_INPUT" > "$PYTHON_OUTPUT"
  mpi_rc=$?
fi
